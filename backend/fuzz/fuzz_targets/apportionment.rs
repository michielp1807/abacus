...
fuzz_target!(|data: FuzzedElectionSummary| {
    match seat_allocation(data.seats, &data.election_summary) {
        Ok(alloc) => {
            ...
            assert_eq!(total_votes, votes_per_party.iter().sum::<u32>() as u64);

            let total_seats = data.seats;
            let seats_per_party = alloc.get_total_seats();
            assert_eq!(
                total_seats,
                seats_per_party.iter().sum::<u64>(),
                "{seats_per_party:?}"
            );
            ...
            let seats_lower_bound = votes_per_party
                .iter()
                .map(|v| total_seats * u64::from(*v) / total_votes)
                .collect::<Vec<_>>();
            assert!(
            ...
            let seats_upper_bound = votes_per_party
                .iter()
                .map(|v| {
                    ((total_seats * u64::from(*v)) as f64 / (total_votes as f64)).ceil() as u64
                        + extra
                })
                .collect::<Vec<_>>();
            assert!(
